...
was then used to classify the test set of the EMNIST data. The accuracy of the model on the test data is \textit{84.0\%} and the error is \textit{0.471}. This is because the model is able to generalise well over unseen data and has not overfit on the training set.}
}
...
\begin{itemize}
    \item Dropout with low coefficient for L1 Penalties
    \item Dropout with low coefficient for L2 Penalties
    \item L1 Penalties and L2 Penalties
    \item L1 \& L2 Penalties with Dropouts for layers with L1 Penalties
    \item L1 \& L2 Penalties with Dropouts for layers with L2 Penalties
    \item L1 \& L2 Penalties with Dropouts for layers with both L1 and L2 Penalties
    \item Penalties only applied to weights
    \item Penalties only applied to biases
\end{itemize}
These experiments would give us an idea about which regularisation methods work well together and what should be the hyperparameters for the combination of these methods. The dropouts control the number of neurons the model is using to train. Controlling this directly controls the capacity of the network and hence it is a crucial tool in preventing overfitting. 
...
in a uniform scale. Dropouts with L2 will lead to a smoother model which will converge in fewer instances while still helping the performance of the model.}
}
...
helping in preventing overfitting. Various experiments were formulated to combine the power of dropout and L1/L2 norms to get the best of all the regularisation methods and help the model performance. These formulations can help us get a good idea about combinations of regularisation methods and their hyperparameters that work well together. Further studies should
