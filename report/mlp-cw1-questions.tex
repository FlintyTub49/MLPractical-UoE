...
signifying that the model is unable to fit the training data well.\par From these testings, we found that the dropout model with inclusion probability of 0.97 was performing the best on the validation set. This model was then used to classify the test division of the EMNIST data. The accuracy of the model on the test data is \textit{84.52\%} and the 
error is \textit{0.483}. This is because the model is able to generalise well over unseen data and has not overfit on the training set.}
}
...
\youranswer
{Q14 We can extend this experiment by testing a plethora of combinations of the dropout regularisation and L1 \& L2 penalties to fine tune the model further.}
}
...
\begin{table*}[t]
    ...
    \midrule
        \multirow{4}*{Dropout}
                 ...
                 & \textit{\textbf{0.97}} & \textit{\textbf{85.4}} & \textit{\textbf{0.244}} & \textit{\textbf{0.457}}  \\
    \midrule
