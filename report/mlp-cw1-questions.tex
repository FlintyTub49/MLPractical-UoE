...
\youranswer
{We can extend this experiment by testing a plethora of combinations of the dropout regularisation and L1 \& L2 penalties to fine tune the model further. We have already seen that each regularisation method is used for some specific task and has its own advantages and disadvantages. By combining these methods, we can try to mitigate the disadvantages 
of one method by using the other. We can test the following combinations of the regularisation methods: \par \textbf{Dropout + L1 penalty} - This combination can be used to reduce the number of features in the model as L1 penalty will shrink the coefficients of the less important features to 0 and the dropout will
prevent the model from overfitting the training data. \par \textbf{Dropout + L2 penalty} - This combination can be used to reduce the variance of the model and hence prevent overfitting. The L2 penalty will shrink the coefficients of the features to a smaller scale and the dropout will prevent the model from overfitting the training data. 
\par \textbf{Dropout + L1 + L2 penalty} -}
}
...
helping in preventing overfitting. Various experiments were formulated to combine dropout and L1/L2 norms to get the best of both regularisation methods and help the model performance. <MODEL PERFORMANCE>. Further studies should focus on using combined regularisation methods like Elastic Net to prevent overfitting. Early stopping is another approach that can be 
explored to combat overfitting in machine learning models.}
}
