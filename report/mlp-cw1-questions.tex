...
\youranswer
{slightly results in improved model performance overall. If either or both the width
and depth are increased substantially, the model starts overfitting the training data and returns low accuracy scores for any unseen data.}
}
...
\youranswer
{the model which was just using dropout regularisation was performing the best. It was able to properly learn from the training data and return high accuracy scores on the testing data while keeping the error of the model low.}
}
...
Overfitting a model also wastes a lot of resources since it requires a lot of computational power to create a model which will be unable to handle new data points efficiently.}
}
...
This usually happens when the model has more room than what is needed for capturing the patterns in the data. If the dataset is small or simple, the basic essence of it can be captured with a small model itself.
...
Thus it is essential to choose an appropriate model for the data provided to us, otherwise choosing a complex model with more network capacity will result in overfitting of the data.}
}
...
\youranswer
% Describe figure 1a
{the training accuracy of the model increases rapidly in the first few epochs and then gradually slows down as the epochs continue to run. Even though the validation accuracy increases at a good rate in the first few epochs, it starts flattening out after a point and then slowly starts to 
...
\youranswer
{the final validation accuracy of all the models is very close (Figure 2a) indicating that the models perform very similarly. In Figure 2b, if we look at the train and the validation error, we can see the difference in the performance of the models. The training error of the model with 32 hidden units is the highest 
and the one with 128 hidden units is the lowest. This would ideally mean that the model with 128 neurons would perform the best. But if we look at the validation error, we can see the 128 units model has the highest validation error. This means that model has overfit the training data and is performing relatively
...
This model is relatively less complex and is performing better than a model with more width.}
}
...
accuracy of around 80\% (Table 2). The rise of accuracy is very steep in the first 10 epochs, followed by slow gradual improvement in training accuracy and barely any change in the validation accuracy. \par The error behaves very differently and gives a good understanding of the models. In Figure 3b, we can see that the error of all the models on 
the training set approaches 0, but on the validation set starts increasing as the epochs increase. This is the point of overfitting; the magnitude of the error indicates how much the model has learned the training data. The model with the highest depth has the highest capacity, thereby learning the train set the best, resulting 
...
on the test set of \textit{84.55\%}. Although the accuracy is higher, this cannot be a factor in choosing a model since we cannot check the accuracy scores on the test set in real life scenarios.}
}
...
of one method by using the other. Some of the combinations for the models that we can use are ($\alpha$ signifies the learning rate):
\begin{itemize}
...
the model with learning rate $1e-4$, L1 penalty $5e-4$, L2 penalty $1e-3$ and dropout inclusion probability of 0.9 performed the best giving an validation accuracy of \textit{83.5\%}}
}
